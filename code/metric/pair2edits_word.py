...
src_path = 'flacgec/src.txt'  # 不带sid的source
tgt_path = 'flacgec/trg.txt'  # 不带sid的target ————！！！每次terminal运行记得改参数！！！
# sid_path = sys.argv[3] # sid或带sid的source
# pair2edits_word.py flacgec/src.txt flacgec/trg.txt > flacgec/ref.txt
# perl evaluation.pl flacgec/gector-out.txt flacgec/gector-report.txt flacgec/ref.txt  ———— 测试
...
with open(src_path, encoding='utf-8') as f_src, open(tgt_path, encoding='utf-8') as f_tgt:
    ...
    for i in range(len(lines_src)):
        id, src_line = lines_src[i].split('\t')[0], lines_src[i].split('\t')[1].split('\n')[0]
        tgt_line = ''.join(lines_tgt[i].replace(',', '，').split())
